...
print("=== HEADSET MICROPHONE TRANSCRIBE DIAGNOSTIC ===")

devices = sd.query_devices()
headset_input_idx = 2  # The auto-discovered headset mic
dev_name = devices[headset_input_idx]["name"]
samplerate = 44100

print(f"[+] Using Headset Microphone: {dev_name} (Index: {headset_input_idx})")
print(f"[+] Samplerate: {samplerate}")

print("\n[*] RECORDING START: Speak clearly into your headset mic for 4 seconds now!")
time.sleep(0.5)
...
try:
    ...
    # Save a WAV copy for playback check
    with wave.open("scratch/test_headset.wav", 'wb') as wav_file:
        wav_file.setnchannels(1)
        wav_file.setsampwidth(2)
        wav_file.setframerate(samplerate)
        wav_file.writeframes(full_audio.astype(np.int16).tobytes())
